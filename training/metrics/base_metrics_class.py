...
def calculate_metrics_for_train(label, output):
    ...
    # Accuracy
    # 注意：对于多分类任务，如果输出是logits，需要先argmax
    # 对于二分类，可以直接用prob > 0.5来判断，但这里的实现也是可以的
    prediction = torch.round(prob) if output.size(1) == 1 else torch.max(output, 1)[1]
    correct = (prediction == label).sum().item()
    accuracy = correct / prediction.size(0)
    ...
    # Average Precision - 在单类别时可能报错，我们同样处理
    try:
        ap = metrics.average_precision_score(y_true, y_pred)
    except ValueError:
        ap = 0.5 # 如果无法计算AP，也给一个默认值

    # AUC and EER
    # 检查label是否都为同一类别
    if len(np.unique(y_true)) < 2:
        # 如果只有一个类别，AUC/EER无法计算，直接返回默认值
        auc = 0.5
        eer = 0.5
    else:
        # 正常计算
