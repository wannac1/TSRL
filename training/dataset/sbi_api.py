...
class SBI_API:
    def __init__(self, phase='train', image_size=256, blend_loc='whole_face'):
        ...
        if blend_loc=='whole_face':
            self.hull_list = [0, 1, 2, 3]
        elif blend_loc=='trick_whole_face':
            self.hull_list = [0, 1, 2]
        elif blend_loc=='attribute':
            self.hull_list = [4, 5, 6]
        elif blend_loc=='all':
            self.hull_list= [0, 1, 2, 3, 4, 5, 6]
        elif blend_loc=='whole_att':
            self.hull_list= [0, 1, 2, 3, 7]



    def __call__(self, img, landmark=None,aug=True,return_mask=False,specific_loc=None):
        try:
            assert landmark is not None, "landmark of the facial image should not be None."
            img_r, img_f, mask_f = self.self_blending(img.copy(), landmark.copy(),spe_loc=specific_loc)
            if self.phase == 'train' and aug:
    ...
    def self_blending(self, img, landmark,spe_loc=None):
        ...
        if spe_loc is None:
            hull_type = random.choice(self.hull_list)
        else:
            hull_type = spe_loc
        logging.disable(logging.FATAL)
        mask = random_get_hull(landmark, img, hull_type)
