...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def __init__(self, config=None, mode='train'):
        ...
        # --- ▼▼▼ 新增配置项 ▼▼▼ ---
        self.use_combined_lmdb = config.get('use_combined_lmdb', True)
        self.combined_lmdb_name = "deepfakes_detection_datasets_lmdb"  # 您指定的 LMDB 名称
        # --- ▲▲▲ 新增结束 ▲▲▲ ---
        # Dataset dictionary
        self.image_list = []
        self.label_list = []
        ...
        if mode == 'train':
            ...
            for one_data in dataset_list:
                if self.lmdb:
                    ...
                    if self.use_combined_lmdb:
                        lmdb_path = os.path.join(config['lmdb_dir'], self.combined_lmdb_name)
                    else:
            ...
            if self.lmdb:
                ...
                if self.use_combined_lmdb:
                    ...
                    lmdb_path = os.path.join(config['lmdb_dir'], f"{dataset_name}_lmdb")
                print(f"\n--- [DEBUG] Opening LMDB for TEST: {lmdb_path} ---", file=sys.stderr, flush=True)
                self.env[dataset_name] = lmdb.open(lmdb_path, create=False, subdir=True, readonly=True, lock=False)
    ...
    def load_rgb(self, file_path):
        ...
        if not self.lmdb:
            ...
            # --- 修改开始 ---
            if self.use_combined_lmdb:
                # 如果是合并库，直接用初始化时存好的总名（即 self.dataset_name）
                target_key = self.dataset_name
            else:
                # 否则，按照原逻辑从路径提取（注意处理 Windows/Linux 路径分隔符）
                target_key = file_path.replace('\\', '/').split('/')[0]

            # 防御性代码：如果 target_key 还是不在 env 里，直接取 env 里的第一个可用句柄
            if target_key not in self.env:
                target_key = list(self.env.keys())[0]

            with self.env[target_key].begin(write=False) as txn:
    ...
    def __getitem__(self, index, no_norm=False):
        ...
        MODIFIED: Ensures that video-level tensors are either a complete stack or a single None.
        """
        # --- 之前的逻辑保持不变 ---
        ...
        for image_path in image_paths:
            ...
            try:
                ...
                # 打印到 stderr 并 强制刷新 (flush=True)
                print(f"\n--- FATAL DATASET ERROR ---", file=sys.stderr, flush=True)
                print(f"Error loading image at index: {index}", file=sys.stderr, flush=True)
                print(f"Failed Path: {image_path}", file=sys.stderr, flush=True)
                print(f"Specific Error: {e}", file=sys.stderr, flush=True)
                print(f"-----------------------------", file=sys.stderr, flush=True)

                # 重新抛出异常，让程序彻底崩溃并显示全部信息
                raise e
        ...
        if self.video_level:
            ...
            # 检查landmark和mask是否完整，如果不完整，则整个样本的对应项为None
            if any(lt is None for lt in landmark_tensors):
                landmark_tensors = None
            else:
                landmark_tensors = torch.stack(landmark_tensors, dim=0)

            if any(mt is None for mt in mask_tensors):
                mask_tensors = None
            else:
    ...
    def collate_fn(batch):
        ...
        MODIFIED: Handles a batch of dictionaries from the __getitem__ method.
        """
        # batch 是一个列表，列表中的每个元素都是一个字典
        # 例如: [{'image': tensor, 'label': 0}, {'image': tensor, 'label': 1}, ...]
        if not isinstance(batch[0], dict):
            # 这是一个备用逻辑，以防万一传入了非字典格式，但正常情况下不会触发
            ...
            landmarks = [d['landmark'] for d in batch]
            masks = [d['mask'] for d in batch]
            video_names = [d['video_name'] for d in batch]
            # --- ▲▲▲ 修改结束 ▲▲▲ ---

        # 后续的打包逻辑与您原有的版本几乎一致
        # 我们需要确保能安全地处理批次中可能存在的None值
        ...
        # 安全地打包 landmarks
        if not all(lank is None for lank in landmarks):
            # 过滤掉None值以安全地堆叠
            valid_landmarks = [lank for lank in landmarks if lank is not None]
            if valid_landmarks:
                # 注意：如果批次中部分有landmark部分没有，这里打包后的batch_size会变小
                # 更稳健的做法是填充None值，但需要模型侧支持，目前先按过滤处理
                landmarks = torch.stack(valid_landmarks, dim=0)
            else:
                landmarks = None  # 整个批次都没有有效的landmark
        else:
            landmarks = None  # 整个批次都没有landmark

        # 安全地打包 masks
        if not all(m is None for m in masks):
