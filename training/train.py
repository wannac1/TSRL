...
parser.add_argument('--weighted_loss', action='store_true', default=False, help='enable weighted loss based on confidence')
...
def main():
    ...
    # 添加加权损失配置
    config['trainer'] = config.get('trainer', {})
    config['trainer']['weighted_loss'] = args.weighted_loss
    ...
    # 记录加权损失设置
    logger.info(f"Weighted loss enabled: {config['trainer']['weighted_loss']}")
