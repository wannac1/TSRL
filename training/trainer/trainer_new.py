...
class Trainer(object):
    def __init__(
        ...
        # 添加加权损失配置
        self.weighted_loss = config['trainer'].get('weighted_loss', False)
        self.logger.info(f"Using weighted loss: {self.weighted_loss}")
    ...
    def compute_sample_weights(self, predictions, labels):
        """分段加权策略"""
        with torch.no_grad():
            probs = torch.sigmoid(predictions['prob']).squeeze()
            ...
            weights[error_mask] = 3.0 + 2.0 * (1 - torch.abs(probs[error_mask] - 0.5) * 2)
            
            # 对低置信度的正确样本也给予适当关注
            low_confidence_correct = (correct == 1) & (torch.abs(probs - 0.5) * 2 < 0.3)
            weights[low_confidence_correct] = 1.5
    ...
    def train_step(self, data_dict):
        if self.config['optimizer']['type'] == 'sam':
            for i in range(2):
                ...
                # 应用加权损失（如果需要）
                if self.weighted_loss and i == 0:  # 只在第一次前向时应用权重
                    weights = self.compute_sample_weights(predictions, data_dict['label'])
                    losses['overall'] = (losses['overall'] * weights).mean()
    ...
    def train_epoch(
        ...
        for iteration, data_dict in tqdm(enumerate(train_data_loader),total=len(train_data_loader)):
            ...
            # 记录权重分布信息（每100个iteration）
            if self.weighted_loss and iteration % 100 == 0 and 'prob' in predictions:
                with torch.no_grad():
                    probs = torch.sigmoid(predictions['prob']).squeeze()
                    ...
                    stats = {
                        'weight/confidence_mean': confidence.mean().item(),
                        'weight/confidence_std': confidence.std().item(),
                        'weight/correct_rate': correct.mean().item(),
                        'weight/hard_example_rate': (confidence[correct == 0].mean().item() 
                                                    if (correct == 0).sum() > 0 else 0)
                    }
                    
                    for name, value in stats.items():
                        writer = self.get_writer('train', ','.join(self.config['train_dataset']), 'weight_stats')
                        writer.add_scalar(name, value, global_step=step_cnt)
    ...
    def save_best(self, epoch, iteration, step, losses_one_dataset_recorder, key, metric_one_dataset):
        ...
        if improved:
            ...
            # 仅在存在特征且不是平均指标时保存特征
            if self.config['save_feat'] and 'feat' in metric_one_dataset and key != 'avg':
                self.save_feat('test', metric_one_dataset['feat'], key)
        ...
        for k, v in metric_one_dataset.items():
            if k in ['pred', 'label', 'dataset_dict', 'feat', 'Confusion Matrix']:
                continue
            ...
            try:
                ...
                self.logger.warning(f"Cannot log metric {k}: {str(e)}")
    ...
    def test_epoch(self, epoch, iteration, test_data_loaders, step):
        ...
        for key in test_data_loaders.keys():
            ...
            # 添加特征到metric字典中
            if feature_nps is not None:
                metric_one_dataset['feat'] = feature_nps
        ...
        if len(test_data_loaders.keys()) > 0 and self.config.get('save_avg', False):
            for key in avg_metric:
                if key != 'dataset_dict':
                    avg_metric[key] /= len(test_data_loaders.keys())
            
            # 确保avg_metric不包含特征矩阵
            if 'feat' in avg_metric:
                del avg_metric['feat']
